...
q96 <- stats::quantile(df_raw$Size, probs = c(0.6, 0.8, 0.9, 0.95, 0.96, 1), na.rm = TRUE)[5]
df_raw$high <- ifelse(df_raw$Size >= q96, 'yes', 'no')
...
log2mat <- round(log2(tf_species_matrix), 2)

# Construct the heatmap
df_log2rescaled <- melt(t(log2mat))
colnames(df_log2rescaled) <- c("Species", "TF", "Size")

q75 <- stats::quantile(df_log2rescaled$Size, na.rm = TRUE)[4]
df_log2rescaled$high <- ifelse(df_log2rescaled$Size >= q75, 'yes', 'no')
...
vitis_resc_mat <- log10(vitis_resc_mat)
...
q96.5 <- stats::quantile(df_vitis_resc$Size, probs = c(0.6, 0.8, 0.9, 0.95, 0.965, 1), na.rm = TRUE)[5]
df_vitis_resc$high <- ifelse(df_vitis_resc$Size >= q96.5, 'yes', 'no')
...
plot_vitis <- ggplot(df_vitis_resc, aes(x = TF, y = Species, fill = Size)) +
  geom_tile() +
  scale_fill_gradient2(low = "#bd0e01", mid = "#f3ffce", high = "#00441B", name = "TF size\n(log)") +
  theme_minimal() +
  theme(axis.text.y = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold"),
...
log2_vitis_mat <- round(log2_vitis_mat, 2)

# Construct the heatmap
df_log2vitis_resc <- melt(t(log2_vitis_mat))
colnames(df_log2vitis_resc) <- c("Species", "TF", "Size")

q96.4 <- stats::quantile(df_log2vitis_resc$Size, probs = c(0, 0.25, 0.5, 0.75, 0.9, 0.964, 1), na.rm = TRUE)[6]
df_log2vitis_resc$high <- ifelse(df_log2vitis_resc$Size >= q96.4, 'yes', 'no')
...
wrapped1 <- wrap_plots(species_tree_nice, plot_vitis, widths = c(0.6, 2.4))
wrapped2 <- wrap_plots(species_tree_nice, plot_log2, widths = c(0.6, 2.4))
